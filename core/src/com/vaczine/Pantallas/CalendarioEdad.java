...
public class CalendarioEdad implements Screen{
   ...
   // private Texture img;
    ...
    private TextButton b_volver, b_masEdad, b_menosEdad, b_masVacuna, b_menosVacuna; //crea botones con texto similares a los de swing
    ...
    private int edad=0;
    private int vacuna =1;
    private int posVacuna = 1;


    public CalendarioEdad (VaczineGame game) {
        this.game = game;
        ...
        listaEdad = cal.getEdades();//lista edad cargo la lista de las edades
    ...
    public void show() {
      ...
      //  img = new Texture("fondo.jpg");//imagen
        ...
        b_menosEdad= new TextButton("/\\", estilo);
        b_masEdad = new TextButton("\\/", estilo);
        ...
        m.addBoton(b_menosEdad,1f,1f,40,100,25,400);
        m.addBoton(b_masEdad,1f,1f,40,100,25,100);


        m.addBoton(b_menosVacuna,1f,1f,200,50,170,0);
        m.addBoton(b_masVacuna,1f,1f,200,50,870,0);



        stage.addActor(b_volver);
        stage.addActor(b_masEdad);
        stage.addActor(b_menosEdad);
        ...
        b_masEdad.addListener(new InputListener() {
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
            ...
            public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
                ...
                vacuna=1;
                posVacuna=1;
                if(edad==listaEdad.size()){edad=0;}

            }});

        b_menosEdad.addListener(new InputListener() {
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
            ...
            public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
                ...
                vacuna =1;
                posVacuna=1;
                if(edad<0){edad=listaEdad.size()-1;}
        ...
        b_masVacuna.addListener(new InputListener() {
            ...
            public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
                ...
                vacuna += 2;
                posVacuna+=1;
                if(posVacuna>contarVacunas(listaEdad.get(edad))){posVacuna=1;}
                if (vacuna == listaEdad.get(edad).length) {
                    vacuna = 1;}

                String str = listaEdad.get(edad)[vacuna];

                while (str.equals("null") || str.equals("")) {
                    vacuna += 2;
                    if (vacuna == listaEdad.get(edad).length) {
                        vacuna = 1;posVacuna=1;}

                    str = listaEdad.get(edad)[vacuna];
        ...
        b_menosVacuna.addListener(new InputListener() {
            ...
            public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
                ...
                vacuna -= 2;
                posVacuna-=1;
                if(posVacuna<1){posVacuna=contarVacunas(listaEdad.get(edad));}
                if (vacuna <1) {
                    vacuna =listaEdad.get(edad).length-2;posVacuna=contarVacunas(listaEdad.get(edad));}
    ...
    int contarVacunas(String[] lista){

        int nvacunas=0;

        for (int i=1; i<lista.length; i+=2){

         if(lista[i].equals("null") || lista[i].equals("")){

           nvacunas+=1;
         }
        }

        return ((lista.length-1)/2)-nvacunas;
    }
    ...
    public void render(float delta) {
      ...
      //  batch.end();
        ...
        caja.rect(310, 50,3, 480);//eje de las y
        caja.rect(0, 530,ancho, 3);//eje de las x1
        caja.rect(0, 50,ancho, 3);//eje de las x2
        caja.end();
        ...
        fuente.draw(batch,game.getTx().getCalendarioDeVacunacion()+"-"+ game.getTx().getPorEdad()+" (Argentina)", 110, 580,900,-1,true);
        ...
        if(i==edad){fuente.draw(batch,">", 55, y);}//marca la edad que esta en uso

        fuente.draw(batch,listaEdad.get(i)[0], 80, y);
        ...
        fuente.draw(batch,listaEdad.get(edad)[vacuna], 320, 520);


        //mostrar las indicaciones de cada vacuna

        fuente.getData().setScale(0.8f,0.7f);
        fuente.setColor(255f/255f, 228f/255f, 112f/255f, 1);


        fuente.draw(batch,listaEdad.get(edad)[vacuna+1], 320, 450, 700,-1,true);

        //mostrar cuantas vacunas hay por edad

        fuente.getData().setScale(0.8f,0.8f);

        fuente.draw(batch,game.getTx().getVacunasEncontradas()+" "+posVacuna+"/"+ contarVacunas(listaEdad.get(edad)), 320, 35);
