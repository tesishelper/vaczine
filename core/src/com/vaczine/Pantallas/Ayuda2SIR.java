...
public class Ayuda2SIR implements Screen{
    ...
    private TextButton b_volver, b_vermas, b_vermenos; //crea botones con texto similares a los de swing
    ...
    public Ayuda2SIR(VaczineGame game) {
        this.game = game;
    ...
    public void show() {
      ...
      //  if(game.getMusicaOn()==1){ m.playMusic();}//musica de ambiente
       ...
       // img = new Texture("fondo.jpg");//imagen
        img2 = new Texture("pantalla_Ayuda2SIR_"+game.getIdioma()+".png");//imagen
        //	img.set
        ...
        checkBoxStyle.font = fuente_botones;


        b_volver = new TextButton("<<", estilo);
        b_vermenos = new TextButton(game.getTx().getElModeloSIR(), estilo);
        b_vermas = new TextButton(game.getTx().getMasSobreR0(), estilo);
        ...
        m.addBoton(b_vermenos,0.5f,1f,200,60,410,10);
        m.addBoton(b_vermas,0.5f,1f,200,60,612,10);
        ...
        b_vermas.addListener(new InputListener() {
            ...
            public void touchUp (InputEvent event, float x, float y, int pointer, int button) {
                m.playClick();
            //    m.stopMysic();
                game.setScreen(new Ayuda3SIR(game));
            }});

        b_vermenos.addListener(new InputListener() {
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
    ...
    public void render(float delta) {
        ...
        if(verCaja==true){
            ...
            //caja.rect(m.Xsize(40), m.Ysize(80), m.Xsize(20), m.Ysize(10));
            // caja.end();

            fuente.getData().setScale(2,2);
            batch.begin();
            if (stage.getKeyboardFocus() == tf_dias){m.addTexto(batch,fuente,tf_dias.getText(), 420,410);}
